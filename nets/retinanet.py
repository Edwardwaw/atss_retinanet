...
from nets.common import CR, FPN, CBR, CGR
...
class RetinaRegHead(nn.Module):
    def __init__(self, in_channel, inner_channel=256, num_anchors=9, num_layers=4):
        ...
        self.centerness = nn.Conv2d(inner_channel, self.num_anchors * 1, 3, 1, 1)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.normal_(m.weight, std=0.01)
                if m.bias is not None:
    ...
    def forward(self, x):
        x = self.bones(x)
        x_reg = self.reg(x)
        x_centerness = self.centerness(x)
        bs, _, h, w = x_reg.shape
        x_reg = x_reg.permute(0, 2, 3, 1).contiguous() \
            .view(bs, h, w, self.num_anchors, 4) \
            .view(bs, -1, 4)
        # x_centerness = x_centerness.permute(0, 2, 3, 1).contiguous() \
        #     .view(bs, h, w, self.num_anchors, 1) \
        #     .view(bs, -1, 1)
        x_centerness = x_centerness.permute(0, 2, 3, 1).contiguous() \
            .view(bs, h, w, self.num_anchors, 1) \
            .view(bs, -1)
        return x_reg, x_centerness
...
class RetinaHead(nn.Module):
    ...
    def forward(self, xs):
        cls_outputs = list()
        reg_outputs = list()
        centerness_outputs = list()
        for j, x in enumerate(xs):
            cls_outputs.append(self.cls_head(x))
            reg_out, center_out = self.reg_head(x)
            reg_outputs.append(self.scales[j](reg_out))
            centerness_outputs.append(center_out)
        ...
        if self.training:
            return cls_outputs, reg_outputs, centerness_outputs, self.anchors
        else:
            predicts_list = list()
            for cls_out, reg_out, center_out, anchor in zip(cls_outputs, reg_outputs, centerness_outputs, self.anchors):
                ...
                center_out=center_out.unsqueeze(-1)

                predicts_out = torch.cat([scale_reg, center_out, cls_out], dim=-1)
...
#     mcls_output, mreg_output, mcenter_output, manchor = net(input_tensor)
#     for cls_out, reg_out, center_out, anchor_out in zip(mcls_output, mreg_output, mcenter_output, manchor):
#         print(cls_out.shape, reg_out.shape, center_out.shape, anchor_out.shape)
#     net.eval()
